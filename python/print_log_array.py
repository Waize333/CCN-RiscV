...
def print_labeled_softmax(probabilities, num_classes=10):
    """
    Print softmax probabilities with digit labels (0-9)
    """
    if len(probabilities) != num_classes:
        print(f"Warning: Expected {num_classes} probabilities, got {len(probabilities)}")
    
    print("Classification Results:")
    for i, prob in enumerate(probabilities):
        print(f"Digit {i}: {prob:.6f}")
    
    # Print the predicted class (highest probability)
...
if __name__ == "__main__":
    ...
    # softmax_output = [0.000184, 0.009035, 0.025199, 0.073991, 0.171232, 0.145150, 0.145042, 0.143389, 0.143389, 0.143389]
    # print_labeled_softmax(softmax_output)
